...
class Order(Base):
    ...
    payer_id: Mapped[user_fk_nn]
    payee_id: Mapped[user_fk_nn]
    created_at: Mapped[created_at_type]

    payer: Mapped["User"] = relationship(
        foreign_keys=[payer_id], back_populates="orders_placed"
    )
    payee: Mapped["User"] = relationship(
        foreign_keys=[payee_id], back_populates="orders_received"
    )
